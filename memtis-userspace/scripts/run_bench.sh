#!/bin/bash
set -xve
if [ -z $MEMTIS_USERSPACE_ROOT ]; then
    echo "Set env variables first:"
    echo "   cd memtis-userspace; source set_env.sh"
    exit
fi
...
function func_memtis_setting() {
    sudo sh -c "/usr/bin/echo 199 > /sys/kernel/mm/htmm/htmm_sample_period"
    sudo sh -c "/usr/bin/echo 100007 > /sys/kernel/mm/htmm/htmm_inst_sample_period"
    sudo sh -c "/usr/bin/echo 1 > /sys/kernel/mm/htmm/htmm_thres_hot"
    sudo sh -c "/usr/bin/echo 2 > /sys/kernel/mm/htmm/htmm_split_period"
    sudo sh -c "/usr/bin/echo 100000 > /sys/kernel/mm/htmm/htmm_adaptation_period"
    sudo sh -c "/usr/bin/echo 2000000 > /sys/kernel/mm/htmm/htmm_cooling_period"
    sudo sh -c "/usr/bin/echo 2 > /sys/kernel/mm/htmm/htmm_mode"
    sudo sh -c "/usr/bin/echo 500 > /sys/kernel/mm/htmm/htmm_demotion_period_in_ms"
    sudo sh -c "/usr/bin/echo 500 > /sys/kernel/mm/htmm/htmm_promotion_period_in_ms"
    sudo sh -c "/usr/bin/echo 4 > /sys/kernel/mm/htmm/htmm_gamma"
    ###  cpu cap (per mille) for ksampled
    sudo sh -c "/usr/bin/echo 30 > /sys/kernel/mm/htmm/ksampled_soft_cpu_quota"

    if [[ "x${CONFIG_NS}" == "xoff" ]]; then
	sudo sh -c "/usr/bin/echo 1 > /sys/kernel/mm/htmm/htmm_thres_split"
    else
	sudo sh -c "/usr/bin/echo 0 > /sys/kernel/mm/htmm/htmm_thres_split"
    fi

    if [[ "x${CONFIG_NW}" == "xoff" ]]; then
	sudo sh -c "/usr/bin/echo 0 > /sys/kernel/mm/htmm/htmm_nowarm"
    else
	sudo sh -c "/usr/bin/echo 1 > /sys/kernel/mm/htmm/htmm_nowarm"
    fi

    if [[ "x${CONFIG_CXL_MODE}" == "xon" ]]; then
	${DIR}/scripts/set_uncore_freq.sh on
	sudo sh -c "/usr/bin/echo 'enabled' > /sys/kernel/mm/htmm/htmm_cxl_mode"
    else
	${DIR}/scripts/set_uncore_freq.sh off
	sudo sh -c "/usr/bin/echo 'disabled' > /sys/kernel/mm/htmm/htmm_cxl_mode"
    fi
